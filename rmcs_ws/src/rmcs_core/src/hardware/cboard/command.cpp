...
class Command : public rmcs_executor::Component, public rclcpp::Node {
public:
  Command()
  ...
  void update() override {
    ...
    auto &dynamic_part_above =
        package_above.dynamic_part<PackageDjiMotorControlPart>();
    ...
    auto &dynamic_part_below =
        package_below.dynamic_part<PackageDjiMotorControlPart>();
...
private:
  static void send(serial::Serial &serial, Package &package) {
    ...
    package.verify_part() = calculate_verify_code(
        package.buffer, size - package.verify_part_size());
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::cboard::Command,
