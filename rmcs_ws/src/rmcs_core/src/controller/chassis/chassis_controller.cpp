...
public:
  ChassisController()
    ...
    publisher_ =
        this->create_publisher<std_msgs::msg::Float64>("/armor/angle", 20);
  }
  ...
  void update_wheel_velocities(Eigen::Vector2d move) {
    ...
    if (spinning_) {
      add_wheel_velocity_for_spinning(velocities, 0.4 * wheel_speed_limit,
                                      0.2 * wheel_speed_limit);
    } else if (following_) {
      ...
      double velocity_for_spinning =
          std::clamp(following_velocity_controller_.update(err),
                     -wheel_speed_limit, wheel_speed_limit);
      add_wheel_velocity_for_spinning(velocities, velocity_for_spinning,
                                      0.6 * wheel_speed_limit);
    }
  ...
  static inline void
  calculate_wheel_velocity_for_forwarding(double (&velocities)[4],
                                          Eigen::Vector2d move) {

    double right_oblique =
        wheel_speed_limit * (-move.y() * cos_45 + move.x() * sin_45);
    double left_oblique =
        wheel_speed_limit * (move.x() * cos_45 + move.y() * sin_45);
  ...
  constexpr static inline void add_wheel_velocity_for_spinning(
    ...
    if (velocity_for_spinning > 0) {
      ...
      double velocity_remaining =
          wheel_speed_limit - max_velocity_for_forwarding;
      if (velocity_for_spinning > velocity_remaining) {
        if (velocity_remaining > min_acceptable_absolute_velocity_for_spinning)
          ...
          k = (wheel_speed_limit - velocity_for_spinning) /
              max_velocity_for_forwarding;
      }
      ...
      double velocity_remaining =
          -wheel_speed_limit - min_velocity_for_forwarding;
      if (velocity_for_spinning < velocity_remaining) {
        if (velocity_remaining < -min_acceptable_absolute_velocity_for_spinning)
          velocity_for_spinning = velocity_remaining;
        else
          k = (-wheel_speed_limit - velocity_for_spinning) /
              min_velocity_for_forwarding;
      }
    }
