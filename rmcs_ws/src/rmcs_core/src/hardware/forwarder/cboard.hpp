...
class CBoard {
public:
  explicit CBoard(const rclcpp::Logger &logger, uint16_t product_id = 0x5740,
                  uint16_t vendor_id = 0x0483)
      : logger_(logger) {
    if (!init(vendor_id, product_id)) {
      throw std::runtime_error{
          "Failed to init usb transfer for cboard, see log for detail."};
    }
...
protected:
  virtual void can1_receive_callback(uint32_t can_id, uint64_t can_data,
  ...
  virtual void can2_receive_callback(uint32_t can_id, uint64_t can_data,
                                     bool is_extended_can_id,
                                     bool is_remote_transmission,
                                     uint8_t can_data_length) {
    (void)can_id;
  ...
  virtual void uart1_receive_callback(const std::byte *uart_data,
                                      uint8_t uart_data_length) {
    (void)uart_data;
    (void)uart_data_length;
  };
  virtual void uart2_receive_callback(const std::byte *uart_data,
                                      uint8_t uart_data_length) {
    (void)uart_data;
    (void)uart_data_length;
  }
  virtual void dbus_receive_callback(const std::byte *uart_data,
                                     uint8_t uart_data_length) {
    (void)uart_data;
    (void)uart_data_length;
  }
...
private:
  bool init(uint16_t vendor_id = 0x0483,
            uint16_t product_id = 0x5740) noexcept {
    int ret;
    ...
    if (!libusb_device_handle_) {
      RCLCPP_ERROR(logger_, "Failed to open device (vid=0x%x, pid=0x%x)",
                   vendor_id, product_id);
      return false;
    }
    FinalAction close_device_handle{
        [this]() { libusb_close(libusb_device_handle_); }};
    ...
    libusb_fill_bulk_transfer(
        ...
        [](libusb_transfer *transfer) {
          static_cast<CBoard *>(transfer->user_data)
              ->usb_receive_complete_callback(transfer);
        },
        this, 0);
  ...
  void usb_receive_complete_callback(libusb_transfer *transfer) noexcept {
    ...
    FinalAction resubmit_transfer{[this, transfer]() {
      ...
      if (ret != 0) [[unlikely]] {
        if (ret == LIBUSB_ERROR_NO_DEVICE)
          ...
          RCLCPP_ERROR(
              logger_,
              "Failed to re-submit receive transfer: %d. Terminating...", ret);
        std::terminate();
      }
    }};
    ...
    if (iterator == sentinel) [[unlikely]] {
      RCLCPP_ERROR(logger_, "USB receiving error: No data! status=%d",
                   transfer->status);
      return;
    }
    ...
    FinalAction debug_print_buffer{[this, transfer]() {
      ...
      for (int i = 0; i < transfer->actual_length; i++)
        sprintf(&hex_string[i * 3], "%02x ",
                static_cast<uint8_t>(receive_buffer_[i]));
      RCLCPP_ERROR(logger_, "Buffer (len=%d): %s", transfer->actual_length,
                   hex_string);
    }};

    if (transfer->status != LIBUSB_TRANSFER_COMPLETED) [[unlikely]] {
      RCLCPP_ERROR(logger_,
                   "USB receiving error: Transfer not completed! status=%d",
                   transfer->status);
      return;
    }

    if (*iterator != std::byte{0xAE}) [[unlikely]] {
      RCLCPP_ERROR(logger_, "USB receiving error: Unexpected header: 0x%02x!",
    ...
    while (iterator < sentinel) {
      ...
      auto field_id =
          std::launder(reinterpret_cast<Header *>(iterator))->field_id;
  ...
  void read_can_buffer(std::byte *&buffer,
                       decltype(&CBoard::can1_receive_callback) callback) {
    bool is_extended_can_id;
    bool is_remote_transmission;
    uint8_t can_data_length;
    uint32_t can_id;
    ...
    auto &header =
        *std::launder(reinterpret_cast<const CanFieldHeader *>(buffer));
    ...
    if (is_extended_can_id) {
      auto &ext_id =
          *std::launder(reinterpret_cast<const CanExtendedId *>(buffer));
      buffer += sizeof(ext_id);
      can_id = ext_id.can_id;
      can_data_length = header.has_can_data ? ext_id.data_length + 1 : 0;
    } else {
      auto &std_id =
          *std::launder(reinterpret_cast<const CanStandardId *>(buffer));
    ...
    (this->*callback)(can_id, can_data, is_extended_can_id,
                      is_remote_transmission, can_data_length);
  }

  void read_uart_buffer(std::byte *&buffer,
                        decltype(&CBoard::uart1_receive_callback) callback) {
  ...
  template <typename Functor> struct FinalAction {
    constexpr explicit FinalAction(Functor clean)
        : clean_{clean}, enabled_(true) {}
  ...
  void handle_libusb_event() noexcept {
    timeval timeout{
        0, 500'000}; // 0.5s timeout to prevent (very rare) stuck on exit
...
public:
  explicit TransmitBuffer(CBoard &cboard, unsigned int alloc_transfer_count)
      : cboard_(cboard), free_transfers_(alloc_transfer_count) {
    ...
    free_transfers_.push_back_multi(
        [this, &cboard]() {
          ...
          if (!transfer)
            throw std::bad_alloc{};
          libusb_fill_bulk_transfer(
              transfer, cboard.libusb_device_handle_, out_endpoint,
              new unsigned char[64], 1,
              [](libusb_transfer *transfer) {
  ...
  bool add_can1_transmission(uint32_t can_id, uint64_t can_data,
                             bool is_extended_can_id = false,
                             bool is_remote_transmission = false,
                             uint8_t can_data_length = 8) noexcept {
    return add_can_transmission(CommandId::CAN1, can_id, can_data,
                                is_extended_can_id, is_remote_transmission,
                                can_data_length);
  }

  bool add_can2_transmission(uint32_t can_id, uint64_t can_data,
                             bool is_extended_can_id = false,
                             bool is_remote_transmission = false,
                             uint8_t can_data_length = 8) noexcept {
    return add_can_transmission(CommandId::CAN2, can_id, can_data,
                                is_extended_can_id, is_remote_transmission,
                                can_data_length);
  }

  bool add_uart1_transmission(const std::byte *uart_data,
                              uint8_t uart_data_length) {
    return add_uart_transmission(CommandId::UART1, uart_data, uart_data_length);
  }

  bool add_uart2_transmission(const std::byte *uart_data,
                              uint8_t uart_data_length) {
    return add_uart_transmission(CommandId::UART2, uart_data, uart_data_length);
  }

  bool add_dbus_transmission(const std::byte *uart_data,
                             uint8_t uart_data_length) {
...
private:
  bool add_can_transmission(CommandId field_id, uint32_t can_id,
                            uint64_t can_data, bool is_extended_can_id,
                            bool is_remote_transmission,
  ...
  bool add_uart_transmission(CommandId field_id, const std::byte *uart_data,
                             uint8_t uart_data_length) {

    std::byte *buffer = try_fetch_buffer(
        sizeof(UartFieldHeader) + (uart_data_length > 15) + uart_data_length);
    if (!buffer)
      return false;
      ...
      if (!front) [[unlikely]] {
        if (!transfers_all_busy_)
          RCLCPP_ERROR(cboard_.logger_,
                       "Failed to fetch free buffer: All transfers are busy!");
  ...
  bool trigger_transmission_nocheck() {
    return free_transfers_.pop_front([this](libusb_transfer *transfer) {
      ...
      if (ret != 0) [[unlikely]] {
        if (ret == LIBUSB_ERROR_NO_DEVICE)
          RCLCPP_ERROR(cboard_.logger_, "Failed to submit transmit transfer: "
